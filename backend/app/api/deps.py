...
async def require_admin(x_admin_pin: str | None = Header(default=None)) -> None:
    """Gate admin routes behind the shared PIN.

    This is an explicit, changeable decision (ADMIN_PIN env), not an
    assumed auth model. Revisit if a real login is needed later.
    """
