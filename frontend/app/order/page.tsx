  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        setState({
          kind: "error",
          message: "Please double-check your details — the email looks off, or the order is empty.",
        });
      } else {
        setState({ kind: "error", message: "Something went wrong submitting your order. Please try again." });
      }
    } catch {
      setState({
        kind: "error",
          ...
          <div className="container" style={{ maxWidth: "640px" }}>
            ...
            <div className="subscription-box">
              ...
              <p>
                ...
                to confirm the details. Your reference is{" "}
                <strong>#{state.id.slice(0, 8)}</strong>.
              </p>
              <button
                type="button"
            ...
            >
              ...
              <ul style={{ listStyle: "none" }}>
                {menuItems.map((it) => (
                  ...
                  >
                    <span>{it.name}</span>
                    <span style={{ color: "var(--fresh-green)", fontWeight: 700 }}>{money(it.price_cents)}</span>
                  </li>
                ))}
              </ul>
            </div>
