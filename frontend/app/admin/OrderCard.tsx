...
import { money, type MenuWeek, type Order, type StructuredItem } from "./types";
...
export default function OrderCard({
  ...
  function startEdit() {
    const items = order.structured_items ?? [];
    setRows(
      items.length > 0
  ...
  async function saveEdit() {
    const byName = new Map((menu?.items ?? []).map((m) => [m.name.trim().toLowerCase(), m]));
    const items: StructuredItem[] = rows
      .map((r, idx) => {
        const name = r.name.trim();
        const found = byName.get(name.toLowerCase());
        const original = order.structured_items?.[idx];
        return {
          name,
          ...
          unit_price_cents: found ? found.price_cents : null,
        };
      })
      .filter((it) => it.name !== "" && Number.isFinite(it.quantity) && it.quantity >= 1);
    await patch({ structured_items: items, correction_note: note.trim() || null });
    setEditing(false);
  }
    ...
    <div className="menu-item" style={cardStyle}>
      ...
      {order.delivery_confidence === "low" && !order.delivery_date && (
        ...
        >
          ...
          — the customer was vague{order.delivery_notes ? ` ("${order.delivery_notes}")` : ""}. Set the
          date:
              ...
              {rows.map((row, i) => (
                <div key={i} style={{ display: "flex", gap: "0.4rem", marginBottom: "0.4rem" }}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setRows((p) =>
                        p.map((r, idx) => (idx === i ? { ...r, quantity: e.target.value } : r)),
                      )
                    }
